  ...
  def __init__(self,
    ...
    if not label_lookup_path:
      label_lookup_path = os.path.join(
          '/data/pca/nn_robust_attacks/tmp/imagenet', 'imagenet_2012_challenge_label_map_proto.pbtxt')
    if not uid_lookup_path:
      uid_lookup_path = os.path.join(
          '/data/pca/nn_robust_attacks/tmp/imagenet', 'imagenet_synset_to_human_label_map.txt')
...
def readimg(ff):
  #f = "/data/pca/data/hairdryer_img/"+ff
...
class ImageNet:
  def __init__(self):
    ...
    pool = Pool(8)
    #r = pool.map(readimg, os.listdir("/data/pca/data/hairdryer_img/")[:20])
